...
interface ProgressDao {
    ...
    @Query("SELECT * FROM progress WHERE dateShown = :date LIMIT 1")
    suspend fun getByDate(date: String): ProgressEntity?

    @Query("SELECT * FROM progress ORDER BY dateShown DESC LIMIT 1")
    suspend fun getLatest(): ProgressEntity?

    @Query("SELECT * FROM progress WHERE cycleNumber = :cycle ORDER BY positionInCycle ASC")
    ...
    @Query("SELECT * FROM progress ORDER BY dateShown DESC")
    ...
    @Query("SELECT * FROM progress WHERE opened = 1 ORDER BY dateShown DESC")
