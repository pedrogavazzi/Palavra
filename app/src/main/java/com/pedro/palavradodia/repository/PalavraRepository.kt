 ...
 * compartilhado por todas as instâncias do repositório. Isso evita a condição de corrida em
 * que cliques rápidos na aba "Hoje" (ou a notificação disparando ao mesmo tempo que o app é
 * aberto) geravam leituras/escritas concorrentes e faziam a tela piscar com dados inconsistentes.
 */
class PalavraRepository(private val context: Context) {
    ...
    /**
     * Linha do tempo completa: uma entrada por dia, do início do primeiro ciclo até o fim do
     * ciclo atual (incluindo dias futuros que ainda não tiveram palavra sorteada). A data de
     * cada entrada corresponde ao agendamento da notificação diária. Palavras não aprendidas
     * NUNCA são reveladas — a entrada aparece, mas sem o texto da palavra.
     */
    suspend fun getAgenda(): List<AgendaEntry> {
        ...
        if (all.isNotEmpty()) {
            ...
            while (!d.isBefore(firstDate)) {
                ...
                if (dayEntries.isNullOrEmpty()) {
                    entries.add(AgendaEntry(d, null, false, null, notGenerated = true, future = false))
                } else {
                    for (p in dayEntries.sortedBy { it.wordIndexInDay }) {
                        val word = if (p.opened) wordDao.getById(p.wordId) else null
                        entries.add(AgendaEntry(d, p.cycleNumber, p.opened, word, notGenerated = false, future = false))
                    }
                }
                d = d.minusDays(1)
            }
        ...
        if (latestCycle != null) {
            ...
            while (!f.isAfter(cycleEnd)) {
                futureEntries.add(AgendaEntry(f, latestCycle.cycleNumber, false, null, notGenerated = false, future = true))
                f = f.plusDays(1)
            }
        }

        return futureEntries.reversed() + entries
    }
