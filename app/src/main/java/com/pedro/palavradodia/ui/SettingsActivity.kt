...
class SettingsActivity : BaseActivity() {
    ...
    private fun setupResetCycle() {
        binding.btnResetCycle.setOnClickListener {
            AlertDialog.Builder(this)
                .setTitle("Reiniciar ciclo atual?")
                .setMessage("O ciclo volta a contar a partir de hoje (dia 1 de 7). Suas palavras já aprendidas continuam salvas — isso não apaga nada, só reinicia a contagem dos dias e cancela um teste pendente deste ciclo, se houver.")
                .setPositiveButton("Reiniciar") { _, _ ->
                    lifecycleScope.launch {
                        val ok = repo.resetCurrentCycle()
                        Toast.makeText(
                            this@SettingsActivity,
                            if (ok) "Ciclo reiniciado" else "Ainda não há um ciclo para reiniciar",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
