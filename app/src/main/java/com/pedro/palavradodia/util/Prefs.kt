...
class Prefs(context: Context) {
    ...
    val fontScale: Float
        get() = when (fontSizeOption) {
            0 -> 1.0f   // pequena: um pouco maior que o padrão anterior
            2 -> 1.35f  // grande: bem maior, ainda legível
            else -> 1.15f // média: perceptivelmente maior que o padrão do sistema
        }

    /** Usa as constantes de AppCompatDelegate.MODE_NIGHT_*. Padrão: claro (o app ainda não
     *  tem um tema escuro totalmente refinado, então não herda o modo escuro do sistema
     *  automaticamente — o usuário pode escolher "Escuro" ou "Automático" manualmente). */
    var themeMode: Int
        get() = sp.getInt("theme_mode", AppCompatDelegate.MODE_NIGHT_NO)
